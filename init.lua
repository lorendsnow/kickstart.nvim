...
-- Reload luasnips without closing
vim.keymap.set('n', '<leader><leader>s', '<Cmd>lua require("luasnip.loaders.from_lua").load { paths = "./snippets" }<CR>')
...
vim.api.nvim_create_autocmd({ 'LspAttach', 'BufEnter', 'BufWritePost', 'BufWritePre' }, {
  ...
  group = vim.api.nvim_create_augroup('disable-inlay-hints', { clear = true }),
  pattern = { '*.c', '*.cpp', '*.h', '*.hpp' },
